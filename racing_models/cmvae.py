...
class CmvaeDirect(Model):
    def __init__(self, n_z, gate_dim=3, res=96, trainable_model=True): #changed gate_dim default value from 4 to 3
        super(CmvaeDirect, self).__init__()
        # create the base models:
        self.q_img = dronet.Dronet(num_outputs=n_z*2, include_top=True)
        self.p_img = decoders.ImgDecoder()
        self.p_R = transformer.NonLinearTransformer()
        #self.p_Theta = transformer.NonLinearTransformer() # took out theta
        ...
        self.Psi_params = Lambda(lambda x: x[:, 1]) # took out theta and changed indexing
        self.Phi_params = Lambda(lambda x: x[:, 2])
    ...
    def call(self, x, mode):
        ...
        eps = random_normal(tf.shape(stddev))
        z = means + eps * stddev
        r_params, psi_params, phi_params = self.extract_gate_params(z) # took out theta_params
        if mode == 0:
            gate_recon = tf.keras.layers.concatenate([self.p_R(r_params), self.p_Psi(psi_params), self.p_Phi(phi_params)], axis=1) # took out p_Theta
            img_recon = self.p_img(z)
            return img_recon, gate_recon, means, stddev, z
        elif mode == 1:
            img_recon = self.p_img(z)
    ...
    def extract_gate_params(self, z):
        ...
        return r_params, psi_params, phi_params # took out theta_params
